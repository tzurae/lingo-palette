...
Evidence Packs are published from the packaged origin `https://tzurae.github.io/lingo-palette-evidence/`. Trusted extension code constructs paths only from supported language and version identifiers. CI builds and verifies candidates; publication requires Product Owner or repository-maintainer approval through the protected `evidence-pack-production` environment.
