...
5. Generate, gate, and schedule review items independently for all five Knowledge Dimensions. Usage-fit claims require matching sense/context evidence; grammar-pattern claims require source-recorded frames, usage notes, or repeated POS-aware corpus attestations; collocation claims require a documented corpus, explicit co-occurrence window, association statistic, and a raw count of at least five. Productive-use prompts exist only for opted-in Learning Items and always require an overt response plus layered Review Judgment. Approved Review Items and resulting Review Evidence pin the exact dimension-specific source authority. Complete a Review Session of up to five approved due items with no same-item repetition. A shortened session is available when only one to four unique Learning Items are due; not-yet-due items are never pulled forward to fill it.

Review Preparation runs sequentially in the background only for enabled Knowledge Dimensions approaching due need. Candidate generation and its independent evaluator reserve one unit of background work together; all background reservations and reconciled usage share a hard 30% sub-limit of both daily token and applicable estimated-cost budgets, leaving the remaining 70% available to foreground Reading Flow work. Durable jobs record queued, running, paused, attempt, target-generation, and replacement state across service-worker suspension. Offline and budget-blocked work pauses without consuming an attempt; transient provider failures make at most three persisted attempts, then require an explicit Learner resume. Only an approved result atomically enters the eligible pool and replaces its dimension schedule. Evidence Pack activation or rollback excludes stale approvals immediately and fans revalidation out by Learning Item and Knowledge Dimension without rewriting old provenance.
